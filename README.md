...
[![codecov](https://codecov.io/gh/61Ss/CPT304-CW1-Budget-App-Group59/branch/main/graph/badge.svg)](https://codecov.io/gh/61Ss/CPT304-CW1-Budget-App-Group59)
...
### Enabling the Codecov badge for a fresh repo

1. Sign in at [codecov.io](https://codecov.io) with the GitHub account that owns the repo and grant access to this repository (one-click for public repos, no token needed).
2. Push to GitHub. The CI workflow runs, uploads `lcov.info`, and Codecov will start serving the badge SVG at the URL referenced above.
3. (Private repos only) Add `CODECOV_TOKEN` as a repository secret in *Settings → Secrets and variables → Actions*; the workflow already passes it through.
